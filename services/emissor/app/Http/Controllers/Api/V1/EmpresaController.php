...
class EmpresaController extends Controller
{
    public function show(Request $request, Empresa $empresa): JsonResponse
    {
        ...
        return response()->json(['data' => $this->serialize($empresa)]);
    }
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $empresas = $request->user()
            ->empresas()
            ->with(['certificado', 'numeracoes', 'configuracaoFiscal'])
            ->orderBy('razao_social')
            ->get()
            ->map(function (Empresa $e) {
                $e->garantirConfiguracaoFiscal();
                $e->load('configuracaoFiscal');
    ...
    public function update(UpdateEmpresaRequest $request, Empresa $empresa): JsonResponse
    {
        ...
        return response()->json(['data' => $this->serialize($empresa->fresh(['certificado', 'numeracoes', 'configuracaoFiscal']))]);
    }
    ...
    private function serialize(Empresa $empresa): array
    {
        ...
        return [
            ...
            'ativa' => $empresa->ativa,
            'emite_nfe' => (bool) $empresa->emite_nfe,
            'emite_nfce' => (bool) $empresa->emite_nfce,
            'emite_nfse' => (bool) $empresa->emite_nfse,
            'onboarding_concluido' => (bool) $empresa->onboarding_concluido,
            'csc_id' => $empresa->csc_id,
            'csc_configurado' => filled($empresa->csc_id) && filled($empresa->csc_token),
            'resp_tec' => [
            ...
            'configuracao_fiscal' => $cfg?->toApiArray(),
