...
use App\Services\Integracoes\Mecanica\MecanicaNfePayloadMapper;
...
class MecanicaNfeController extends Controller
{
    /**
    ...
    public function emitir(
        Request $request,
        MecanicaNfePayloadMapper $mapper,
        ...
        if ($ie === '' || $ie === 'ISENTO' || $ie === 'ISENTA' || $ieDigits === '') {
            return response()->json([
                'mensagem' => 'Inscrição Estadual do emitente inválida/ausente. '
                    .'Atualize a IE real da Mecânica Bedendo no emissor (empresa) e em Configurações → Fiscal. '
    ...
    public function emitirNfse(
        Request $request,
        MecanicaNfsePayloadMapper $mapper,
        ...
        $validator = Validator::make($request->all(), [
            ...
            'ordemNumero' => ['nullable', 'string'],
            'serie' => ['nullable', 'integer', 'min:1'],
            'tomador' => ['nullable', 'array'],
            'tomador.nome' => ['required_with:tomador', 'string'],
            'tomador.documento' => ['required_with:tomador', 'string'],
            'destinatario' => ['nullable', 'array'],
            'servico' => ['nullable', 'array'],
            'servico.codigoServicoLc116' => ['nullable', 'string'],
            'servico.aliquotaIss' => ['nullable', 'numeric'],
            'servico.valorServico' => ['nullable', 'numeric', 'gt:0'],
        ...
        try {
            $payload = $mapper->map($request->all(), $empresa);
            $result = $nfse->emitir($empresa, $payload);
        } catch (InvalidArgumentException $e) {
            return response()->json(['mensagem' => $e->getMessage()], 422);
        } catch (Throwable $e) {
            report($e);
        ...
        return response()->json([
            ...
            'anexoSimples' => $payload['anexoSimples'] ?? null,
        ]);
    }

    /**
     * Emite NF-e (peças) e/ou NFS-e (serviços) a partir de uma OS mista.
     * Falha parcial não aborta a outra emissão.
     */
    public function emitirOrdem(
        Request $request,
        ItensOsSplitter $splitter,
        MecanicaNfePayloadMapper $nfeMapper,
        MecanicaNfsePayloadMapper $nfseMapper,
        AutorizacaoService $autorizacao,
        NfseEmissaoService $nfse
    ): JsonResponse {
        $validator = Validator::make($request->all(), [
        ...
        $itens = is_array($request->input('itens')) ? $request->input('itens') : [];
        $partes = $splitter->split($itens);
        $referenciaId = $request->input('referenciaId');
        ...
        if ($partes['servicos'] !== []) {
            $nfseResult = $this->emitirNfseParte(
                $request->all(),
                $partes['servicos'],
                $empresa,
                $nfseMapper,
        ...
        if ($nfeResult === null && $nfseResult === null) {
            return response()->json([
                'mensagem' => 'Nenhum item válido para emissão.',
            ], 422);
        }

        return response()->json([
            'referenciaId' => $referenciaId,
            'ordemId' => $request->input('ordemId'),
            'ordemNumero' => $request->input('ordemNumero'),
            'nfe' => $nfeResult,
            'nfse' => $nfseResult,
        ]);
    }
     ...
     */
    private function emitirNfeParte(
        array $input,
        array $pecas,
        Empresa $empresa,
        MecanicaNfePayloadMapper $mapper,
        ...
        if (! $empresa->certificado) {
            return [
                'status' => 'rejeitada',
                'mensagem' => 'Cadastre o certificado A1 da empresa no emissor antes de emitir NF-e.',
                'chaveAcesso' => null,
                'protocolo' => null,
                'numero' => null,
                'serie' => null,
        ...
        if ($ie === '' || $ie === 'ISENTO' || $ie === 'ISENTA' || $ieDigits === '') {
            return [
                'status' => 'rejeitada',
                'mensagem' => 'Inscrição Estadual do emitente inválida/ausente para emissão de NF-e.',
                'chaveAcesso' => null,
                'protocolo' => null,
                'numero' => null,
                'serie' => null,
        ...
        $inputPecas = $input;
        $inputPecas['itens'] = $pecas;

        try {
            $payload = $mapper->map($inputPecas, $empresa);
        } catch (InvalidArgumentException $e) {
            return [
                'status' => 'rejeitada',
                'mensagem' => $e->getMessage(),
    ...
    /**
     * @param  array<string, mixed>  $input
     * @param  list<array<string, mixed>>  $servicos
     * @return array<string, mixed>
     */
    private function emitirNfseParte(
        array $input,
        array $servicos,
        Empresa $empresa,
        MecanicaNfsePayloadMapper $mapper,
        ...
        try {
            $payload = $mapper->map($input, $empresa, $servicos);
            $result = $nfse->emitir($empresa, $payload);
        } catch (InvalidArgumentException $e) {
            return [
                'status' => 'rejeitada',
