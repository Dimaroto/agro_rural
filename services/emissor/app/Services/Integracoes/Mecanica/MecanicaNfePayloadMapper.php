...
/**
 * Converte o payload simplificado do app Mecânica Bedendo
 * para o formato interno de emissão NF-e / NFC-e (NFePHP).
 */
class MecanicaNfePayloadMapper
{
    public function map(array $input, Empresa $empresa): array
    {
        ...
        foreach ($itensIn as $item) {
            ...
            $csosn = preg_replace('/\D/', '', (string) ($item['csosn'] ?? ''));
            $cstIcms = preg_replace('/\D/', '', (string) ($item['cstIcms'] ?? ''));
            $empresa->loadMissing('configuracaoFiscal');
            $cfg = $empresa->configuracaoFiscal;
            $pisDefault = $cfg?->cst_pis ?? ($empresa->crt === 3 ? '01' : '49');
            $cofinsDefault = $cfg?->cst_cofins ?? ($empresa->crt === 3 ? '01' : '49');
            $cstPis = preg_replace('/\D/', '', (string) ($item['cstPis'] ?? $pisDefault));
            $cstCofins = preg_replace('/\D/', '', (string) ($item['cstCofins'] ?? $cofinsDefault));
            $cstIpi = preg_replace('/\D/', '', (string) ($item['cstIpi'] ?? ''));
            $pIcms = $this->nullableFloat($item['aliquotaIcms'] ?? null);
            $pPis = $this->nullableFloat($item['aliquotaPis'] ?? $cfg?->p_pis);
            $pCofins = $this->nullableFloat($item['aliquotaCofins'] ?? $cfg?->p_cofins);
            $pIpi = $this->nullableFloat($item['aliquotaIpi'] ?? null);

            $icms = [
                'orig' => $origem !== '' ? (int) $origem : 0,
                'CSOSN' => $empresa->crt === 3 ? null : ($csosn !== '' ? $csosn : ($cfg?->csosn_padrao ?? '102')),
                'CST' => $empresa->crt === 3 ? ($cstIcms !== '' ? $cstIcms : ($cfg?->cst_icms_padrao ?? '00')) : null,
            ];
            if ($pIcms === null && $empresa->crt === 3 && $cfg?->p_icms_interno) {
                $pIcms = (float) $cfg->p_icms_interno;
            }
            if ($pIcms !== null && ($empresa->crt === 3 || ($icms['CSOSN'] ?? '') === '900')) {
                $icms['modBC'] = 3;
                $icms['vBC'] = $vProd;
                $icms['pICMS'] = $pIcms;
            ...
            $cstPis = $cstPis !== '' ? $cstPis : $pisDefault;
            $cstCofins = $cstCofins !== '' ? $cstCofins : $cofinsDefault;
            ...
            $itens[] = [
                ...
                'CFOP' => $cfop !== '' ? $cfop : ($cfg?->cfop_interno ?? '5102'),
                'CEST' => $cest !== '' ? $cest : ($cfg?->cest_padrao ?: null),
        ...
        if ($pctAprox === null || $pctAprox <= 0) {
            $empresa->loadMissing('configuracaoFiscal');
            $cfg = $empresa->configuracaoFiscal;
            $pctAprox = $cfg?->percAproxTributosPara('mercadoria') ?? 13.45;
        }
