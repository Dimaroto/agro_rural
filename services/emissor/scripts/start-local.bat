...
set "LOGDIR=%LOCALAPPDATA%\Edem Software\Mecanica Bedendo\logs"
...
if defined ALREADY (
  ...
  if /I "%MECANICA_EMISSOR_HIDDEN%"=="1" exit /b 0
...
if not defined PHP (
  ...
  if /I "%MECANICA_EMISSOR_HIDDEN%"=="1" exit /b 1
  pause
  exit /b 1
)
...
set "MB_TMP=%LOCALAPPDATA%\Edem Software\Mecanica Bedendo\tmp"
...
set "SECRETS_DIR=%LOCALAPPDATA%\Edem Software\Mecanica Bedendo\config"
if exist "%SECRETS_DIR%\.env" (
  copy /Y "%SECRETS_DIR%\.env" "%EMISSOR_ROOT%\.env" >nul
  if exist "%SECRETS_DIR%\.mecanica_token.txt" copy /Y "%SECRETS_DIR%\.mecanica_token.txt" "%EMISSOR_ROOT%\.mecanica_token.txt" >nul
)
...
if exist "%~dp0ensure-mecanica-token.php" (
  >>"%LOG%" echo ensure-mecanica-token ...
  if defined PHPRC (
    "%PHP%" -c "%PHPRC%" "%~dp0ensure-mecanica-token.php" >>"%LOG%" 2>&1
  ) else (
    "%PHP%" "%~dp0ensure-mecanica-token.php" >>"%LOG%" 2>&1
  )
)
...
if defined PHPRC (
  if /I "%MECANICA_EMISSOR_HIDDEN%"=="1" (
...
if /I "%MECANICA_EMISSOR_HIDDEN%"=="1" exit /b !RC!
if not "!RC!"=="0" pause
exit /b !RC!
