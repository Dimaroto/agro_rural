...
/**
 * Garante um token Sanctum em .mecanica_token.txt (+ LocalAppData\config)
 * para o app Flutter emitir NF-e sem Unauthenticated.
 *
 * Uso:
 *   runtime\php\php.exe scripts\ensure-mecanica-token.php
 *   runtime\php\php.exe scripts\ensure-mecanica-token.php --force
 */
...
$tokenFile = $emissorRoot.DIRECTORY_SEPARATOR.'.mecanica_token.txt';
$localApp = getenv('LOCALAPPDATA') ?: '';
$configDir = $localApp !== ''
    ? $localApp.DIRECTORY_SEPARATOR.'Edem Software'.DIRECTORY_SEPARATOR.'Mecanica Bedendo'.DIRECTORY_SEPARATOR.'config'
    : '';
$configToken = $configDir !== '' ? $configDir.DIRECTORY_SEPARATOR.'.mecanica_token.txt' : '';
...
$plain = $user->createToken('mecanica-app')->plainTextToken;
