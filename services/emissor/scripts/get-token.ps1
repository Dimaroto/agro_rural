...
$login = Invoke-RestMethod -Uri "$BaseUrl/api/v1/auth/login" -Method Post -ContentType 'application/json' -Body (@{
    email = $Email
    password = $Password
    device_name = 'mecanica'
} | ConvertTo-Json)

$token = $login.token
$token | Set-Content (Join-Path $PSScriptRoot '..\.mecanica_token.txt') -NoNewline
...
Write-Host ''
$tokenPath = (Resolve-Path (Join-Path $PSScriptRoot '..\.mecanica_token.txt')).Path
