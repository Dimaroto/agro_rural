...
function New-SecretsPayloadZipBytes {
    ...
    try {
        ...
        if ($TokenPath -and (Test-Path $TokenPath)) {
            $tokenFinal = Join-Path $tmp '.mecanica_token.txt'
...
function Get-SecretsConfigDir {
    $base = Join-Path $env:LOCALAPPDATA 'Edem Software\Mecanica Bedendo'
...
function Sync-EnvToEmissorRoot {
    ...
    $canonicalToken = Join-Path $configDir '.mecanica_token.txt'
    $rootEnv = Join-Path $EmissorRoot '.env'
    $rootToken = Join-Path $EmissorRoot '.mecanica_token.txt'
