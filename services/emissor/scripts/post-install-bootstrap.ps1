...
function Exit-Bootstrap([int]$Code, [string]$Message = '') {
    ...
    if ($Code -ne 0) {
        ...
        Write-BootstrapLog '--- FALHA no bootstrap. Log: %LOCALAPPDATA%\Edem Software\Mecanica Bedendo\logs\bootstrap.log ---' 'Yellow'
        if (-not $NoPause) {
            Write-Host ''
...
Write-BootstrapLog '=== Mecanica Bedendo - bootstrap do emissor ===' 'Cyan'
...
if (-not (Test-Path $envFile) -and (Test-Path $encFile)) {
    ...
    try {
        ...
        $srcToken = Join-Path $tmpSecrets '.mecanica_token.txt'
        if (Test-Path $srcToken) {
            Copy-Item -Force $srcToken (Join-Path $configDir '.mecanica_token.txt')
        }
...
if (Get-Command Protect-EnvFileAcl -ErrorAction SilentlyContinue) {
    ...
    $tokenFile = Join-Path $EmissorRoot '.mecanica_token.txt'
...
$ensureToken = Join-Path $PSScriptRoot 'ensure-mecanica-token.php'
if (Test-Path -LiteralPath $ensureToken) {
    Write-BootstrapLog 'Gerando token Sanctum para o app Flutter ...'
    & $php $ensureToken
    if ($LASTEXITCODE -ne 0) {
        Write-BootstrapLog '[AVISO] ensure-mecanica-token falhou — gere em Configuracoes > Integracao app.' 'Yellow'
    } else {
        Write-BootstrapLog 'Token salvo em .mecanica_token.txt (cole no Flutter se necessario).' 'Green'
    }
}
