...
class ConfiguracaoController extends Controller
{
    ...
    public function createToken(Request $request): RedirectResponse
    {
        ...
        $token = $user->createToken('agro-app')->plainTextToken;

        $tokenFile = base_path('.agro_token.txt');
        @file_put_contents($tokenFile, $token);
        $configDir = (getenv('LOCALAPPDATA') ?: '').DIRECTORY_SEPARATOR.'Agro Rural Zortea'.DIRECTORY_SEPARATOR.'emissor'.DIRECTORY_SEPARATOR.'config';
        if ($configDir !== '' && str_contains($configDir, 'Agro Rural Zortea')) {
            if (! is_dir($configDir)) {
